...
type Activity = {
  ...
  activity_date: string; company_name: string;
};
...
export default function RepStatsPage() {
  ...
  async function load() {
    ...
    const { data: repData } = await supabase.from("reps")
      .select("id,name,monthly_target_sqm").eq("auth_user_id", user.id).single();
    if (!repData) return;
    setRep(repData);
  ...
  // Top active companies this month
    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Personal performance overview</p>
        </div>
      ...
      <div className="card p-6">
        ...
        <div className="w-full bg-gray-100 rounded-full h-3 mb-2">
          <div
            className={`h-3 rounded-full transition-all ${pct >= 100 ? "bg-green-500" : pct >= 60 ? "bg-brand-blue" : pct >= 30 ? "bg-amber-500" : "bg-red-400"}`}
            style={{ width: `${pct}%` }}
          />
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div className="card p-5">
          <div className="font-semibold text-gray-900 text-sm mb-4">Interaction Breakdown</div>
          {interactionTotal === 0 ? (
            ...
            <div className="space-y-2.5">
              {Object.entries(interactionMap).sort((a,b) => b[1]-a[1]).map(([type, count]) => {
                const pctBar = Math.round((count / interactionTotal) * 100);
                return (
                  <div key={type}>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-gray-700">{type}</span>
                      <span className="text-gray-500">{count} ({pctBar}%)</span>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-1.5">
                      <div className="h-1.5 rounded-full bg-brand-blue" style={{ width: `${pctBar}%` }} />
                    </div>
                  </div>
                );
              })}
