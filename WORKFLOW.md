...
| **`end_state` is now derivable from `lost_reason_id`.** With the type down to `'lost'`, `projects_loss_state` and `projects_loss_detail` plus `assertLossReason` make the two a biconditional in both directions — a project has an end state exactly when it has a loss reason. The column could go, and the form's Open/Lost select becomes "pick a reason or do not". Not taken in the `S31` session: it was the third widening of one slice's scope and the first two were deliberate | `src/db/schema.ts`, `src/lib/projects.ts` | Undecided — a simplification, not a defect |
...
| Handover and dormancy reassignment accept **any** active user as recipient — only `isActive` is checked and the picker is `listActiveUsers()` unfiltered — so a company book can land on an Executive or a Super Admin. `S9` names "a rep, a desk rep, or the coordinator". The code is wider than the rule, not narrower. **Investigated in the `S9` session and not fixed, because `S9`'s list cannot be written as a flag test.** `roles` holds twelve flags and every one names an *act* — assign, share, export, set targets, sees all reps, dispatch, approve quotation, impersonate, manage users, set credit split, approve delete, resolve duplicate. **None names holding a company**, and no column on `users` distinguishes a rep either. Worse than absent: **Desk Rep and Marketing carry an identical flag vector** — `can_assign` and nothing else — in the seed and in the live table, where a `GROUP BY` over all twelve puts them in one group. Any predicate over `roles` is a function of that vector, so **no flag test can admit the desk rep `S9` names and refuse marketing, which it does not.** The partition that *is* expressible is "not one of the three elevated roles", and **five flags give it identically** — `sees_all_reps`, `can_manage_users`, `can_set_targets`, `can_approve_delete`, `can_resolve_duplicate` — none of them about holding a company. Choosing one is a decision, not a derivation, and it would leave marketing admitted, so `S9` keeps its `[CHANGE]`. **The data is clean under every candidate**: 0 of 529 live memberships, 0 of 286 project owners, 0 of 74 credit-split rows and 0 of 767 dispatch reps are held by anyone any of those tests would refuse — all 529 memberships are Sales Rep (517) or Sales Coordinator (12), both named by `S9`. **Handover is one of four**, and the other three are not this row's: the direct dispatch's rep (`dispatches.ts:1012`, which checks **no** `isActive` at all — only `can_dispatch` to name somebody other than self), a credit split's members (`credit-splits.ts:205-210`, existence only) and a share's recipient (`sharing.ts:229-233`, `isActive`). Neither company creation nor project creation picks: both write `session.user.id` | `src/lib/team.ts:233-241`, `src/lib/dormancy.ts:199-207`, `src/lib/authz.ts:1125-1133` | **Blocked on a founder decision**, which is one of three: add the flag `S9` implies and seed it on the three roles it names; amend `S9` to a list the flags can express, which means saying whether marketing may hold a company since it is indistinguishable from the desk rep; or name one of the five proxies and accept that marketing stays admitted. Session 21 cannot start until one is chosen |
...
| `quotation_threads.cancelled_at` is written by `cancelThread` and read by nothing; 0 rows carry one. `cancelled_by_user_id` beside it *is* read. **Session 17 reopened `cancelThread` and left it there deliberately** — the task excluded this section — and it is now also the counter-example `dispatches.cancellation_reason` cites for having no `cancelled_at` of its own: the audit row carries the actor and the moment | `src/db/schema.ts` | Still open. Its own session, or the next dead-structure sweep |
...
| **Dead enum values.** `record_type.quotation_version` is set nowhere — the `comments` CHECK excludes it, `SHARED_RECORD_TYPES` excludes it, and the three tables that could carry it are dead. ~~`project_end_state.dormant`~~ **is gone**, dropped in migration `0024` alongside `won`: the type was being rebuilt for `S31` anyway, no rule ever defined a dormant project, and 0 rows carried one. Nothing offers it on a screen — the form's select maps `PROJECT_END_STATES`, which is now one value. (`form_factor.coil` is its own row above) | `src/db/schema.ts` | `quotation_version` undecided — needs a rule or a drop. The projects half is done |
