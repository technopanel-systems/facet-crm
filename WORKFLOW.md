...
| 40 | **`D20`'s enforcement.** The scripts-off **operability** section in `verify:routes` — every field an action requires must be a native, focusable control carrying that name — plus the two fixes it needs to land green: the checkbox primitive (**one file**, `ui/checkbox.tsx`) and the city field. Everything else the walk names is printed as a note and becomes a `§5` row owned by its screen's session | `D20` `S15` |
...
| **A Saudi company cannot be registered with scripts off, and that ships today.** `Combobox` is the one control `D20` ever named as a real JavaScript exception, and under the rewritten `D20` it is **enablement, not an exception**: scripts off, it renders a `<button>` and an **empty hidden input** (`combobox.tsx:145`), so nothing can fill it. `S15` makes a city mandatory for Saudi Arabia, so the company form cannot be submitted at all — **not a degraded screen, a broken one**. It is invisible to the walk because `verify:routes` §13 writes the registration body itself: a POST no human could have produced still passes, which is exactly the half `D20`'s new operability assertion adds. The same control carries the project form's city, the report form's city and — undeclared, `AD19` — the report form's **company** picker | `src/components/ui/combobox.tsx:145`, `companies/company-form.tsx:225`, `projects/project-form.tsx:131`, `reports/report-form.tsx:155`, `:292`, `SPEC.md` `S15`, `DESIGN.md` `D20` | **Session 40**, and it is why 40 runs before any more screens are built. The value must post from a native control; the search over it is the enhancement |
| **Every checkbox in FACET is inert with scripts off — seven call sites, one file.** `ui/checkbox.tsx` is Radix's primitive, and the server sends a `<button role="checkbox">` with an `aria-hidden` input beside it at `tabIndex:-1` and `pointer-events:none`, so **only the default value can ever post**. `setCreditSplit` refuses an empty `userIds` (`credit-splits.ts:189`) and that control is its only writer, so **a credit split cannot be set**; the handover buckets, `inProduction`, `committed` `S29`, the report's signals `S43`, a comment's mentions and the comment editor are the same shape, degrading to whatever was already ticked. Radix renders the hidden input on purpose — *"so that events bubble to forms without JS (SSR)"* — but bubbling is not operating. **The fix is one primitive, not seven screens**: a styled native `<input type="checkbox">`, every call site unchanged | `src/components/ui/checkbox.tsx`, `src/lib/credit-splits.ts:189`, seven call sites, `DESIGN.md` `D20` | **Session 40** — one of the seven is a required field, so the operability assertion cannot land green without it |
