...
| **The coordinator's approval queue has no signal for which requests need reading.** `S72` says she *checks* a request and approves it, and at 12–15 a day that is 12–15 screens opened to check any of them. The thing that would let a row say "these three differ from their quotation, the other ten match" is `S120`'s difference flag, which is unbuilt. Session 13 declined to invent a substitute, and put approve and refuse on the detail screen only — a per-row button would let her approve without opening it, which is the opposite of the rule's verb | `S72` `S120`, `/dispatches` | Session 15, which builds `S120`. Watch it at the pilot |
