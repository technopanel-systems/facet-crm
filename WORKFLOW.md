...
| **12 of 66 quotation versions carry no product lines at all, 6 of them issued**, and `S60` says a quotation always keeps at least one. Found by hand while counting unpriced lines for `S120`'s plan, on a database freshly reset — so it is reachable through today's code rather than old residue. Not investigated: the count is the whole of what is known | `src/lib/quotations.ts`, `S60` | Own session, or the next audit. Nothing in `S120` depends on it — a lineless version simply differs from any dispatch raised against it |
