...
| `S15` says the region "is derived from the city and shown read-only. The rep is never asked for it." With no city chosen the company form renders a region `<SelectField>` and `regionForCity(null, chosen)` returns the rep's pick, so a Saudi company with no city carries a hand-typed region. **44 of 844 companies already do.** Any region-sliced figure mixes derived and typed values. `projects.region` has the identical shape | `src/lib/lookups.ts:86`, `companies/company-form.tsx:253-280`, `projects/project-form.tsx:158-175` | Own session — it is a rule-versus-code conflict, not a form tweak |
