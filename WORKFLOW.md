...
| **`SPEC §16` asks that project names be decided *before the projects slice*, and the projects slice has now shipped without it.** *"Companies and contacts have one name field (`S12`, `S19`). Projects still have two, and no rule says why. Same reasoning appears to apply."* The **symptom** is fixed — the list header read `NAME (ENGLISH)`, a schema detail in a column header, and is now `common.name` over a cell that has always picked by locale. **The cause is untouched**: `projects.name_en` is `notNull` and `name_ar` is nullable, `lookupName` exists for the pair, the form asks for both, and `FollowUpRow` keeps `anchorNameEn`/`anchorNameAr` *"because the anchor may be a project, which still carries two names"*. Fixing a symptom is how a cause gets buried, so it is a row rather than a closed question. One field would remove a column, a form field, a message key and `lookupName`'s last project caller | `SPEC.md §16`, `src/db/schema.ts` `projects.name_en`/`name_ar`, `src/lib/lookups.ts` `lookupName`, `src/lib/follow-ups.ts` `FollowUpRow` | Open — a founder decision, then a migration. Its own session |
...
| **The linked dispatch route forces a project; the free route forbids one.** `projectForThread` refuses without one (`dispatches.ts:882`); `requestDispatch` writes `projectId = null` unconditionally on the free route (`:1028-1031`) and the action strips the field (`dispatches/actions.ts:127`). **No rule says either.** A rep who used `S50` exactly as intended — quote first, attach the project later — is shown a **required** picker that can be empty, while the `S75` route that legitimately has no project is never asked. Found in the board-rules session as `B2`, recorded here rather than left in a plan file | `src/lib/dispatches.ts:882`, `:1028-1031`, `src/app/[locale]/(app)/dispatches/actions.ts:127` | Open — `S50`'s pending amendment closes it by removing the null case altogether. If that amendment is not taken, this needs a rule of its own |
