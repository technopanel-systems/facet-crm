...
**What has actually shipped, and in this order:** the token slice, the shell
`23`, `seed:demo`, three dashboard slices (`D64` `D65` `D32`–`D34` `D69`), the
companies list, the company detail `D70`, the verify-residue slice `34b`, the
projects board and table `35`, the board's six rules `S132`–`S135`, one name
field on projects `S26`, and `S50`'s two halves. **Four of those were never
sessions in this plan** — `seed:demo`, the dashboard, the companies screens and
the board's rules — and `34b` and `35` ran out of order on purpose. The table
below is what is left, in the order it is now to be done.

**`40` runs first.** No more screens are built on the rewritten `D20` until
there is an assertion behind it.

| # | Session | Rules |
|---|---|---|
| 23 | ~~The shell — seven-item rail, tokens, layout cap, theme, permission boolean~~ **Done.** The rail carries eight items until `28b` deletes `/performance`; `§5` records why moving the item early is the thing that would break | `D5`–`D23` `D49` `D50` |
| 40 | ~~**`D20`'s enforcement.** The scripts-off **operability** section in `verify:routes`~~ **Done.** `§23` scans inside `get()`, the choke point the message-key scan already uses, so all 909 fetches are covered at no extra cost: **5,179 forms, 0 required fields behind JavaScript, 1 note.** `checkbox.tsx` is native and `combobox.tsx` is deleted. **The section proves itself before it is believed** — it is fed the two shapes this slice removed and asserts it catches both, because a section that has only ever been green has not been shown to work | `D20` `S15` |
| 26 | **The quotations list** — grouped, per-object lead cell, whose-move column. `AD16` is settled first: `chain.ts` has no *waiting on the customer* owner to group by. Carries `quotations.ts:261`'s un-normalised search and `AD10`'s stale owner at `paid` | `D25`–`D27` `D2` |
| 41 | **The dispatches list and detail** — the `D24` Detail archetype, and `D26`'s dispatch clause widened from the two states it answers for to the four the screen renders `AD33`. Carries `dispatches.ts:372`'s search | `D24` `D26` `S72` `S88` |
| 28 | **The project detail and the quotation detail.** The company detail shipped in the `D70` slice; these two have not. `thread-lines.tsx`'s four forms behind client state come out here — under the rewritten `D20` they are unreachable with scripts off | `D24` `D2` `D26` `D27` `D20` |
| 27 | **The stream** — replaces `/reports` and `/activity`; comments narrowed to threads and projects. Carries `reports.ts:527`'s search and `D48`'s missing marker `AD7` | `S114` `D45`–`D48` `D30` |
| 28b | **Targets and Performance merged** — one table, one row per rep, the goal and the attainment together, the edit control per row for `can_set_targets`, and `/performance` deleted. Takes the rail to `D49`'s seven, settles `AD20`, and lands **before AUDIT 2** so the audit reads a seven-item rail | `D49` `D58` |

### Phase 2b — What every screen then needs

| # | Session | Rules |
|---|---|---|
| 42 | **Auto-refresh.** One count route, one line per block, sixty seconds, and **nothing beneath the line moves**. Visible rather than silent, because a row that moves under the cursor mid-click is an approval of the wrong dispatch | `D72` `D20` |
| 38 | **Mobile** — rep screens at 375px, because reps work from phones and `D55`/`D56` are unbuilt. `AD8` is decided first: `D56` describes the opposite of what ships, in both halves. Carries `AD34`'s iOS glow and the rail footer below ≈487px | `D55` `D56` |
...
| # | Session | Rules |
|---|---|---|
| 24 | **The waiting list's other half — a deletion, 3,445 lines overdue.** The dashboard replaced them and the old mechanism never came out: `follow-ups.ts` (1,522), `notifications.ts` (1,190), `/follow-ups` (248) and `/notifications` (385) all still ship beside it, against `CLAUDE.md`'s rule that the old mechanism leaves in the same slice. Carries the `notifications.record_type` CHECK and `S90`/`S92`/`S122`'s three decisions | `S86`–`S95` `S91` |
...
| 43 | **Backups — automated, with a restore proved onto a second machine.** Self-hosted on one company Windows PC, backed up to Synology: an untested backup is not a backup, and `AUDIT 3` already asks for the restore rather than for the backup | — |
...
| Search filters `ilike` a raw name, so the same Arabic name is findable on one screen and not another. **Corrected by AUDIT 1: three of six, not five.** `companies.ts`, `projects.ts:134` and `contacts.ts:77` all normalise | `quotations.ts:261`, `dispatches.ts:372`, `reports.ts:527` | **26**, **41** and **27** — each search leaves with its own list: `quotations.ts:261` with the quotations list, `dispatches.ts:372` with the dispatch screens, `reports.ts:527` with the stream |
| ~28 sites need `dir="auto"` per D62 | list cells, option labels, panels — inventory in session 3a's report | Session **47**, with the `SelectField` item below |
...
| **`AD33`. `D26`'s dispatch clause cites a `[BUILD]` rule and covers two of four states.** It cites `S88`, which is `[BUILD]`, for behaviour that ships under `S72`. It answers for a submitted request and an approved dispatch, and has nothing for a draft (owes its rep, `S86`) or a refusal (closed). The code renders all four, so the rule is narrower than the screen | `DESIGN.md` `D26`, `src/app/[locale]/(app)/dispatches/page.tsx:245-263` | Session **41**, with the dispatch screens — one clause |
