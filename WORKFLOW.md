...
| **The same unfiltered `listActiveUsers` names a person at four sites, and they answer the question four different ways.** Handover and dormancy are the row above, now on the narrowed directory. The other three are deliberately untouched, and are one question rather than three unrelated pickers: the **rep on a direct dispatch** (`dispatches.ts:1012`) checks **nothing** — not even `isActive` — only `can_dispatch` to name somebody other than self, and that rep takes the credit `S78`; a **credit split's members** (`credit-splits.ts:205-210`) check **existence only**, so a deactivated colleague can be credited; a **share's recipient** (`sharing.ts:229-233`) checks **`isActive`**, and sharing is arguably not the same question at all — a share grants access to a record somebody else still holds `S100`, so it hands over no book. Neither company creation nor project creation picks: both write `session.user.id` | `src/lib/dispatches.ts:1012`, `src/lib/credit-splits.ts:205-210`, `src/lib/sharing.ts:229-233` | Undecided — needs the founder to say whether the three should answer alike, and if so which answer. Not a defect in any one of them; a defect in there being four |
