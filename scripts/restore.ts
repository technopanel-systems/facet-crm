 ...
 * also asks for the name to be typed back — `db-reset.ts`'s guard, for the same
 * reason: the office PC's `.env` is copied from the same `.env.example`, so no
 * environment variable can tell the two machines apart.
 *
...
/** `db-reset.ts`'s confirmation, for the same reason and in the same shape. */
