 ...
 *
 * **This is the point of the backup slice.** `WORKFLOW.md` §4 row 43 does not
 * ask for a backup, it asks for a restore that has been watched working, and
 * `AUDIT 3` asks for the restore rather than the backup. A dump nobody has
 ...
 *
 * **It refuses to pass on nothing.** A comparison of 0 = 0 across 43 empty
 ...
 *   2. A throwaway `postgres:17-alpine` on an EMPTY VOLUME, started for this
 *      run and removed after it. This is the nearest available stand-in for the
 *      second machine row 43 actually asks about: a same-cluster restore
 *      silently reuses roles, extensions and settings that a real restore has
 *      to bring with it, so on its own it proves less than it looks like it
 *      proves. The image is read off the live container rather than written
 *      here, so the restore can never be tested against a server version the
 *      dump did not come from.
 *
 * Neither is a second machine, and row 43 stays open until one has been done by
 * hand. `README.md` carries those steps.
 */
...
const THROWAWAY = "facet-backup-verify";
 ...
 * this one point shows up in both — which is how the row comparison was watched
 * going red before this check was believed.
 */
...
async function main(): Promise<void> {
  ...
  // Stage 2 — a cluster that has never seen this application.
  ...
  console.log(
    "\nBoth restores match the live database. Neither is a second machine —\n" +
      "that half of WORKFLOW row 43 is a founder step in README.",
  );
}
