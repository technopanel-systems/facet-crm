 ...
 *
 * **`spawnSync` is called with an args array and `shell: false`**, unlike
 * `db-reset.ts`'s `run(command)`. This is a Windows host and `cmd.exe` does not
 * treat `'` as a quote character, so a shell-interpolated `sh -c '…'` would be
 * split on spaces and reach `sh` in pieces. An args array has no shell in it to
 * get this wrong.
 *
 ...
 * `docker compose` has to find `docker-compose.yml`. `loadEnv()` therefore
 * chdirs to the repository root before anything runs.
 */
...
export const DUMP_PREFIX = "facet-";
 ...
 * container sets NODE_ENV=production. Do not add one by symmetry with
 * `db-reset.ts`.
 */
 ...
 * exits 0, which is the failure this whole slice exists to catch. Owner and
