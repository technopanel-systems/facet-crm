/**
 * One consistent dump of the live database.
 * `npm run backup`.
 *
 * **Why this exists, and what it is not.** The Synology NAS already sweeps
 ...
 * written into a folder the NAS is already sweeping. `README.md` says this
 * beside the procedure, because the belief that the NAS has the database
 * covered is the belief that loses it.
 *
 ...
 * worse than no script at all — that is `drizzle-kit`'s failure mode in
 * `CLAUDE.md`, and this is the same shape. So three things are asserted after
 * the dump: a zero exit status, a plausible size, and the `PGDMP` magic that
 * starts a custom-format archive. A file failing any of them is DELETED rather
 * than left to be picked up as "the newest dump" later.
 *
...
/** Host local time, so a nightly 02:00 task sorts the way the founder reads it. */
